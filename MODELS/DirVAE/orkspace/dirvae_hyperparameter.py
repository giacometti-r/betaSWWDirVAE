...
for epoch in range(epochs):
    ...
    if epoch == 0:
        ...
        plt.savefig('/workspace/reconstructed_images/original.png')
    with torch.no_grad():
    ...
    plt.savefig(f'/workspace/reconstructed_images/epoch_{epoch}.png')

    if test_loss.to('cpu').detach().numpy() < best_loss:
        torch.save(model.state_dict(), '/workspace/weights/model.pth')

    if epoch % 25 == 0 and epoch >= 100 and epoch < 200:
