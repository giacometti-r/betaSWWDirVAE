...
class NVDM(object):
    ...
    def __init__(self, 
        ...
        lda=False
        self.x = tf.compat.v1.placeholder(tf.float32, [None, vocab_size], name='input')
        self.mask = tf.compat.v1.placeholder(tf.float32, [None], name='mask')  # mask paddings
        self.warm_up = tf.compat.v1.placeholder(tf.float32, (), name='warm_up')  # warm up
        self.adam_beta1=adam_beta1
        self.adam_beta2=adam_beta2
        self.keep_prob = tf.compat.v1.placeholder(tf.float32, name='keep_prob')
        self.min_alpha = tf.compat.v1.placeholder(tf.float32,(), name='min_alpha')
        # encoder
        with tf.compat.v1.variable_scope('encoder'): 
          self.enc_vec = utils.mlp(self.x, [self.n_hidden], self.non_linearity)
          self.enc_vec = tf.nn.dropout(self.enc_vec,rate=1 - (self.keep_prob))
          ...
          self.analytical_kld = tf.math.lgamma(tf.reduce_sum(self.alpha,axis=1))-tf.math.lgamma(tf.reduce_sum(self.prior,axis=1))
          self.analytical_kld-=tf.reduce_sum(tf.math.lgamma(self.alpha),axis=1)
          self.analytical_kld+=tf.reduce_sum(tf.math.lgamma(self.prior),axis=1)
          minus = self.alpha-self.prior
          test = tf.reduce_sum(tf.multiply(minus,tf.math.digamma(self.alpha)-tf.reshape(tf.math.digamma(tf.reduce_sum(self.alpha,1)),(batch_size,1))),1)
        ...
        with tf.compat.v1.variable_scope('decoder'):
          if self.n_sample ==1:  # single sample
            u = tf.random.uniform((batch_size,self.n_topic))
            with tf.compat.v1.variable_scope('prob'):
                #CDF transform
                self.doc_vec = tf.pow(u*self.alpha*tf.exp(tf.math.lgamma(self.alpha)),1./self.alpha)
                #normalize
                self.doc_vec = tf.compat.v1.div(self.doc_vec,tf.reshape(tf.reduce_sum(self.doc_vec,1), (-1, 1)))
                self.doc_vec.set_shape(self.alpha.get_shape())
            #reconstruction
            if lda:
              logits = tf.math.log(tf.clip_by_value(utils.linear_LDA(self.doc_vec, self.vocab_size, scope='projection',no_bias=True),1e-10,1.0))
            else:
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=self.adam_beta1,beta2=self.adam_beta2)#,beta1=0.99
...
def train(sess, model, 
  ...
  while not stopped:
    ...
    for i in range(alternate_epochs):
      ...
      dec_vars = utils.variable_parser(tf.compat.v1.trainable_variables(), 'decoder')
      phi = dec_vars[0]
      phi = sess.run(phi)
    ...
    if print_ppx<best_print_ana_ppx:
      ...
      tf.compat.v1.train.Saver().save(sess, 'models/improved_model') 
      ...
      if no_improvement_iters>=early_stopping_iters:
          ...
          tf.compat.v1.train.Saver().restore(sess,'models/improved_model')
